...
function updatePlantPanel() {
	...
	if (plantSection) {
		...
		plants.forEach((plant, index) => {
			...
			// Add plant resources
			if (Object.keys(plant.resources).length > 0) {
				const resourcesDiv = document.createElement("div");
				resourcesDiv.className = `plant-resourceslist infostat`;
				...
				for (let resource in plant["resources"]) {
					const keyDiv = document.createElement("div");
					keyDiv.className = `plant-resource infostat`;
					keyDiv.textContent = `${resource}: ${plant.resources[resource]}`;
					resourcesDiv.appendChild(keyDiv);
				}
				plantPanel.appendChild(resourcesDiv);
			}
		});
	}
}
	...
	for (let i = 0; i < numsteps; i++) {
		for (let plant of plants) {
			...
			if (plant.alive == true) {
					...
					for (let key in plant.input[category]) {
						...
						if (
							environment.resources[category][key] -
								plant.input[category][key] * plant.size <
							0
						) {
							plant.healthy = false;
						}
				...
				if (plant.healthy == true) {
					...
					for (let category in plant.output) {
						for (let key in plant.output[category]) {
							...
							environment.resources[category][key] +=
								plant.output[category][key] * plant.size;
						}
					}
					//kept resources
					for (let key in plant.kept) {
						//create new resource if it doesn't exist
						if (!plant.resources[key]) {
							plant.resources[key] = 0;
						}

						plant.resources[key] += plant.kept[key] * plant.size;
					}
				}
				...
				//adjust plant size based on healthy status
				if (plant.healthy == true) {
					plant.size += plant._growthrate;
				}

				//cap plant health
				if (plant.size >= plant._maxsize) {
					plant.size = plant._maxsize;
				}
